# Desktop 4.1.0 upgrade and rollback
...
The installer transaction covers the application directory and its registration only. It does not move or delete `DSH_HOME`, Electron `userData`, project files, conversations, or sessions. Community Desktop 4 uses a new application id and installer GUID. Its one-time 3.x adoption path requires the old private shutdown marker and expected executable, selects that exact installation directory, and retires only the old community registry keys after commit. A foreign or unmarked installation is neither adopted nor removed. Runtime installation damage after a committed update is handled by the updater path; it is not treated as a profile or plugin problem.

`pnpm --filter @linxin666/dsh-desktop test:installer-lifecycle:e2e` compiles and executes the production NSIS hooks against small isolated payloads and unique test registry keys. It covers fresh installation, upgrade, commit failure, and an aborted install section, with Chinese, space, and apostrophe paths plus deletion of the early transaction helper. Both files and registry versions must agree after rollback, and a session sentinel must remain unchanged. This is executable installer-hook coverage; a full old-release-to-new-release overlay in an isolated Windows environment remains a separate release acceptance check. The verifier uses the cached NSIS compiler, or `DSH_NSIS_COMPILER` when explicitly supplied.
