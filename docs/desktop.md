...
Unsigned macOS contributor previews are Apple Silicon (arm64) only. They are not notarized, do not include MinGit, and cannot use in-app updates. Install steps, Gatekeeper bypass (including the macOS 15 removal of Right-click > Open), and the contributor checklist are in the [macOS unsigned preview install guide](macos-preview.md).
