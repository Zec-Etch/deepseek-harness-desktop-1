...
import { toPublicMemoryItem, type MemoryPublicItem } from './core/schema.ts'
...
function genericErrorCode(error: unknown): string {
...
function draftFromArgs(
  ...
  if (typeof args.content !== 'string' || args.content.trim() === '') return undefined
  const scope = args.scope ?? 'global'
  if (scope === 'workspace') {
    if (context.workspaceId === undefined || (args.workspaceId !== undefined && args.workspaceId !== context.workspaceId)) return undefined
    return {
      scope,
  ...
  if (scope === 'session') {
    if (context.sessionId === undefined || (args.sessionId !== undefined && args.sessionId !== context.sessionId)) return undefined
    return {
      scope,
  ...
  if (args.workspaceId !== undefined || args.sessionId !== undefined) return undefined
  return {
    scope: 'global',
    content: args.content,
    ...(args.tags === undefined ? {} : { tags: args.tags }),
...
function searchText(items: readonly MemoryPublicItem[]): string {
...
export function createMemoryTool(service: MemoryService, options: MemoryToolOptions) {
  return defineTool({
    name: 'memory',
    ...
    async execute(args, exec) {
      ...
      if (draft === undefined) return { success: false, operation: args.operation, items: [], errorCode: 'invalid' }
      try {
