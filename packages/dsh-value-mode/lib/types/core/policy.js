...
export function buildSystemPromptGuidance(config, options = {}) {
    ...
    return (`[性价比模式·${strategy === 'saver' ? '更省' : strategy === 'powerful' ? '更强' : '智能平衡'}·专家主控] ` +
        '你是本次会话的专家主控模型，负责理解任务、拆解工作、选择是否派发子代理、审查结果并对最终交付负责。' +
        '常规编码、工具调用和验证可直接完成。' +
        delegationGuidance +
        '派发后要明确任务边界和验收标准，收到子代理结果后自行核对，必要时补充修改与测试；不要为了形式强行创建子代理。');
}
export function buildExpertSystemPrompt(purpose) {
    return ('You are a senior technical expert consultant in Value Mode (性价比模式). ' +
