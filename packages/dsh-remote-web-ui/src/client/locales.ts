...
export const zh = {
  ...
  'status.lanRequired': '当前没有可达的手机地址',
  'status.lanRequiredHint': '桌面版可以在不修改官方 Runtime 的情况下开启一个仅允许移动端配对 API 的本地局域网入口。',
  'lan.address': '电脑局域网地址',
  'lan.enable': '开启本地局域网访问',
  'lan.enabling': '正在开启并重启 Runtime…',
  'lan.enableHint': '仅在可信 Wi-Fi 中开启；Windows 可能首次请求防火墙放行。完整 /api、文件系统和桌面特权接口不会开放。',
  'lan.noAddress': '未找到可用的私有 IPv4 网络，请先连接 Wi-Fi 或有线局域网。',
  'lan.disable': '关闭局域网入口',
  'lan.disabling': '正在关闭…',
  ...
  'settings.requirePairingForLanHint': '该开关仅在“兼容完整 API”模式下生效；开启时非本机回环的 /api 请求必须携带有效配对 Cookie。安全的“仅移动 API”模式会始终拒绝非本机回环的完整 /api，手机通道按配对授权。',
  'settings.publicBaseUrl': '公网地址（可选）',
  'settings.publicBaseUrlHint': '反向代理之后的可访问 origin，例如 https://dsh.example.com。它是唯一来源：二维码链接由它构建，配对围栏按它信任来访主机，隧道也按它对外宣告——改这里就等于整体改掉信任的外部域名。',
  'settings.autoTunnel': '自动公网隧道',
  'settings.autoTunnelHint': '开启后插件会在 profile-local runtime 目录准备并启动所选传输的公网隧道，自动更新公网地址，手机可用公网配对；关闭时使用上方手动填写的公网地址。',
  'settings.tunnelTransport': '公网隧道方式',
  'settings.tunnelTransportHint': 'cloudflare：插件自带的 quick tunnel，每次启动生成随机 trycloudflare.com 地址；ssh：向你自己控制的服务器建立反向 SSH 端口转发，二维码改用你的 HTTPS 域名。默认 cloudflare，既有部署行为不变。',
  'settings.tunnelTransportCloudflare': 'Cloudflare quick tunnel',
  'settings.tunnelTransportSsh': 'SSH 反向隧道（自有服务器）',
  'settings.sshTunnelServer': 'SSH 目标（host 或 user@host）',
  'settings.sshTunnelServerHint': 'ssh 方式必填，例如 root@tunnel.example.com。留空时隧道保持停止，并在面板中报告失败原因。',
  'settings.sshTunnelPort': 'SSH 端口',
  'settings.sshTunnelPortHint': 'SSH 服务端口，默认 22。',
  'settings.sshTunnelRemotePort': '服务器暴露端口',
  'settings.sshTunnelRemotePortHint': '在服务器回环地址上暴露的端口，即该服务器反向代理的上游端口，默认 7788。',
  'settings.sshTunnelKeyPath': 'SSH 私钥路径（可选）',
  'settings.sshTunnelKeyPathHint': '作为 -i 传给 ssh 客户端；留空时使用 ssh 客户端自身的身份（默认密钥或 agent）。',
  'settings.tunnelSurface': '隧道暴露范围',
  'settings.tunnelSurfaceHint': 'mobile：只暴露手机端所需的路径（默认，最窄）；full：暴露完整界面——前端资源加全部运行时路径（含 /api），配对设备看到与桌面相同的应用。full 会把宿主的工具能力（执行命令、读写文件）暴露给持有配对 cookie 的一方，请务必保持「局域网访问要求配对」开启，并使用你独家可控的域名。',
  'settings.tunnelSurfaceMobile': '移动端界面（默认）',
  'settings.tunnelSurfaceFull': '完整界面（含 /api）',
  ...
  'settings.remoteApiMode': '远程完整 API 模式',
  'settings.remoteApiModeHint': '仅移动 API：非本机回环的完整 /api 一律拒绝，仅 /m/api 按设备配对授权；兼容完整 API：保留旧行为，但不提供 Workspace/Session 级资源隔离。',
  'settings.remoteApiModeMobileOnly': '仅移动 API（推荐）',
  'settings.remoteApiModeLegacyFullApi': '兼容完整 API（不隔离）',
  'settings.invalidChoice': '请选择有效选项。',
  // 更新面板（侧边栏更新按钮）。
  'update.label': '检查更新',
  'update.title': '检查更新',
...
export const en = {
  ...
  'status.lanRequired': 'No reachable phone address is configured',
  'status.lanRequiredHint': 'Desktop can open a local LAN entry limited to paired mobile APIs without changing the official Runtime.',
  'lan.address': 'Computer LAN address',
  'lan.enable': 'Enable local LAN access',
  'lan.enabling': 'Enabling and restarting Runtime…',
  'lan.enableHint': 'Use only on trusted Wi-Fi. Windows may ask for firewall access once. Full /api, filesystem and Desktop privilege routes remain closed.',
  'lan.noAddress': 'No private IPv4 network is available. Connect Wi-Fi or Ethernet first.',
  'lan.disable': 'Disable LAN entry',
  'lan.disabling': 'Disabling…',
  ...
  'settings.requirePairingForLanHint': 'This switch only applies in legacy full-api mode. When on, every non-loopback /api request must carry a live paired-device cookie. The safe mobile-only mode always denies non-loopback full /api and authorizes the phone channel by pairing.',
  'settings.publicBaseUrl': 'Public address (optional)',
  'settings.publicBaseUrlHint': 'The origin this deployment is reachable at behind your reverse proxy, e.g. https://dsh.example.com. It is the single source of truth: the QR link is built from it, the pairing fence trusts its host, and the tunnel advertises it — changing it moves the whole trust decision with it.',
  'settings.autoTunnel': 'Auto public tunnel',
  'settings.autoTunnelHint': 'When on, the plugin prepares and runs the selected transport in the profile-local runtime directory and keeps the public address in sync, so a phone anywhere can pair; when off, the manually entered public address above applies.',
  'settings.tunnelTransport': 'Public tunnel transport',
  'settings.tunnelTransportHint': 'cloudflare: the bundled quick tunnel, which mints a random trycloudflare.com address on every start. ssh: a reverse SSH forward to a server you control, so the QR link uses your own HTTPS origin. Cloudflare is the default, so existing deployments behave unchanged.',
  'settings.tunnelTransportCloudflare': 'Cloudflare quick tunnel',
  'settings.tunnelTransportSsh': 'SSH reverse tunnel (own server)',
  'settings.sshTunnelServer': 'SSH destination (host or user@host)',
  'settings.sshTunnelServerHint': 'Required for the ssh transport, e.g. root@tunnel.example.com. While blank the transport stays stopped and the panel reports why.',
  'settings.sshTunnelPort': 'SSH port',
  'settings.sshTunnelPortHint': 'SSH server port; 22 by default.',
  'settings.sshTunnelRemotePort': 'Port exposed on the server',
  'settings.sshTunnelRemotePortHint': 'Port bound on the server loopback, i.e. the upstream the reverse proxy on that server forwards to; 7788 by default.',
  'settings.sshTunnelKeyPath': 'SSH private key path (optional)',
  'settings.sshTunnelKeyPathHint': 'Passed to the ssh client as -i; while blank the client uses its own identity (default keys or agent).',
  'settings.tunnelSurface': 'Tunnelled surface',
  'settings.tunnelSurfaceHint': 'mobile: expose only the paths a phone needs (the default, and the narrowest surface). full: expose the complete interface — the frontend plus every runtime path, including /api — so a paired device sees the same application the desktop does. Full scope hands the host\'s tool surface (command execution, file access) to whoever holds a paired-device cookie: keep "require pairing for LAN access" on and use a domain only you control.',
  'settings.tunnelSurfaceMobile': 'Mobile surface (default)',
  'settings.tunnelSurfaceFull': 'Full interface (includes /api)',
  ...
  'settings.remoteApiMode': 'Remote full-api mode',
  'settings.remoteApiModeHint': 'Mobile-only: deny every non-loopback full /api request and authorize only the paired /m/api channel. Legacy full-api: preserve the old behavior, but without Workspace/Session-level resource isolation.',
  'settings.remoteApiModeMobileOnly': 'Mobile API only (recommended)',
  'settings.remoteApiModeLegacyFullApi': 'Legacy full API (no isolation)',
  'settings.invalidChoice': 'Choose a valid option.',
