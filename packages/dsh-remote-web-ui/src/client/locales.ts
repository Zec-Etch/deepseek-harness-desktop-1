...
export const zh = {
  ...
  'settings.publicBaseUrlHint': '内网穿透的公网地址，如 Cloudflare Tunnel：https://xxx.trycloudflare.com。填写后二维码将生成公网链接，手机不在同一网络也能配对；留空时只有明确允许 all-interface bind 的 DSH build 才支持局域网访问。',
  ...
  'settings.sshTunnelPublicUrl': 'SSH 隧道公网地址（可选）',
  'settings.sshTunnelPublicUrlHint': 'ssh 隧道生效时二维码使用的公网 origin，例如 https://dsh.example.com；留空时回退到上方的“公网地址”。两者都为空时无法生成二维码链接。',
...
export const en = {
  ...
  'settings.publicBaseUrlHint': 'The public URL of a tunnel in front of this server, e.g. a Cloudflare Tunnel: https://xxx.trycloudflare.com. When set, the QR link is built from it so a phone anywhere can pair; leaving it blank supports LAN only when the DSH build explicitly permits all-interface binding.',
  ...
  'settings.sshTunnelPublicUrl': 'SSH tunnel public address (optional)',
  'settings.sshTunnelPublicUrlHint': 'The public origin the QR link is built from while the ssh tunnel is up, e.g. https://dsh.example.com; it falls back to the public address above. With neither set no QR link can be built.',
