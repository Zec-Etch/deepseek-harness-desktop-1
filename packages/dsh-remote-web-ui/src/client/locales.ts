...
export const zh = {
  ...
  'settings.autoTunnelHint': '开启后插件会在 profile-local runtime 目录准备并启动 Cloudflare quick tunnel（正常网络下无需手动安装工具），自动更新公网地址，手机可用公网配对；关闭时使用上方手动填写的公网地址。',
...
export const en = {
  ...
  'settings.autoTunnelHint': 'When on, the plugin prepares and runs Cloudflare quick tunnel in the profile-local runtime directory (normal network access needs no manual tool install) and keeps the public address in sync, so a phone anywhere can pair; when off, the manually entered public address above applies.',
