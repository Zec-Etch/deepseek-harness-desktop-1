...
`tunnelSurface` 决定隧道那个本地面暴露什么。`mobile`（默认）只伺服手机端界面与其数据通道，因此仅限本机的控制类路由永远不会出现在任何 TCP 面上。`full` 则伺服完整应用：前端资源加其余全部运行时路径（含 `/api`），配对设备因此看到与桌面相同的界面——同一个运行时、同一批会话。

full 是刻意的取舍：宿主的工具能力（执行命令、读写文件）会变得对持有配对 cookie 的一方可达，而这正是官方 `--host 0.0.0.0` 守卫拒绝暴露的东西。请保持 `requirePairingForLan` 开启、`publicBaseUrl` 使用你独家可控的域名，并用「停止」切断设备。手机端仍会渲染桌面布局。

配对仍是完整界面的外层门。运行时用自身浏览器会话 cookie 授权 `/api`，该 cookie 由进程启动令牌换发——那是 Electron 管道内部携带、普通浏览器永远拿不到的凭据；缺了它就会出现「重连中」。因此本插件会让**已持有配对 cookie 的设备**去走一次该换发流程（运行时下发 cookie 并 303 到干净的 `/`），未配对的一律收到运行时原本的拒绝。请先用二维码链接完成配对，再打开 `/`。
