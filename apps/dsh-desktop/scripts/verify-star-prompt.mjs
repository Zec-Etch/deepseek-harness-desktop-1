...
try {
  ...
  await firstPrompt.getByText('4.0.0-rc.2 · 社区支持', { exact: true }).waitFor({ state: 'visible' })
  ...
  if (!claimedState.shownVersions?.includes('4.0.0-rc.2')) {
    throw new Error(`4.0.0-rc.2 Star prompt did not persist its once-per-release claim: ${JSON.stringify(claimedState)}`)
  }
  ...
  if (await thirdPage.locator('#dsh-desktop-star-prompt[data-open="true"]').isVisible()) {
    throw new Error('the 4.0.0-rc.2 Star prompt appeared more than once for the same user profile')
  }
