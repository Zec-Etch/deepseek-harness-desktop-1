...
test('package verification consumes the shared critical runtime file contract', async () => {
  ...
  assert.match(source, /const platform = \["@deepseek-ai\/dsh-client", "-store"\]\.join\(""\)/u)
  assert.match(source, /engine = require\(platform\)/u)
  assert.match(source, /const createSnapshotStore = engine\.createSnapshotStore/u)
