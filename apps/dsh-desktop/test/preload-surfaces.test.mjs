...
test('extension preload exposes extension operations without product update actions', async () => {
  ...
  assert.match(source, /allowIncompatible/u)
  assert.match(source, /installMarketPlugin: \(id, allowIncompatible = false\)/u)
  assert.match(source, /extensions:skill-import/u)
  assert.match(source, /extensions:qqbot-bind/u)
  assert.doesNotMatch(source, /agent-team/u)
  ...
  assert.match(source, /openLogs: \(\) => ipcRenderer\.invoke\('extensions:logs-open'\)/u)
...
test('Electron binds each renderer window to its dedicated preload', async () => {
  ...
  assert.match(source, /DOCK_SETTINGS_PRELOAD_PATH/u)
