...
test('NSIS preflight cleans only stale processes owned by the previous install', async () => {
  ...
  assert.match(config, /allowToChangeInstallationDirectory: true/u)
  ...
  assert.doesNotMatch(cleanup, /Add-Type|DshInstaller\.ProcessPath|QueryFullProcessImageName|GetLongPathNameW/u)
  assert.match(cleanup, /function Get-CanonicalPath/u)
  assert.match(cleanup, /Get-Item -LiteralPath \$fullPath -Force -ErrorAction Stop/u)
  assert.match(cleanup, /\$path = \[string\] \$process\.ExecutablePath/u)
